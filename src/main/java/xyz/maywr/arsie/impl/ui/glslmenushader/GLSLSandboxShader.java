...
public class GLSLSandboxShader
{
    ...
    public GLSLSandboxShader( String name, InputStream is ) throws IOException
    {
        ...
        if( linked == 0 )
        {
            Arsie.logger.error( glGetShaderInfoLog( program, glGetProgrami( program, GL_INFO_LOG_LENGTH ) ) );
            return;
        }
    ...
    public int createShader( String check, InputStream inputStream, int shaderType ) throws IOException
    {
        ...
        if( compiled == 0 )
        {
            Arsie.logger.error( glGetShaderInfoLog( shader, glGetShaderi( shader, GL_INFO_LOG_LENGTH ) ) );
            Arsie.logger.error( "Caused by {}", check );
            return 0;
        }
